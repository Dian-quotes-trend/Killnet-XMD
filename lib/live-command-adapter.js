...
/**
 * Register only commands that do not already exist in the legacy registry.
 * Legacy commands remain the single owner of their existing names. Semantic
 * duplicates such as help/menu and del/delete are represented as aliases,
 * keeping one canonical command entry in the menu and registry.
 */
...
function createRegistryFromLegacyMap(commands) {
  ...
  for (const [rawName, item] of commands.entries()) {
    ...
    if (aliasTarget && commands.has(aliasTarget)) {
      deferredAliases.push([name, aliasTarget]);
      continue;
    }
  ...
  for (const [alias, target] of deferredAliases) {
