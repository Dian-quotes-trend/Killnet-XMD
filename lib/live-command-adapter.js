...
function installDynamicMenu(registry) {
  ...
  menu.handler = async (ctx) => {
    ...
    const headings = {
      CORE: '🧭 GENERAL', INFO: 'ℹ️ INFORMATION', CONFIG: '⚙️ CONFIGURATION', AUTOMATION: '🤖 AUTOMATION',
      GROUP: '👥 GROUP', MODERATION: '🛡️ MODERATION', ADMIN: '🔐 ADMIN — OWNER ONLY', UTILITY: '🧰 UTILITIES',
    };
    ...
    for (const category of order) {
      ...
      for (const item of items) lines.push(`│ • ${prefix}${item.name} » ${item.description}${item.ownerOnly ? ' 🔒' : ''}`);
      lines.push('╰───────────────╯', '');
    }
