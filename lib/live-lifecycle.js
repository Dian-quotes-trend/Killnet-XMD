...
function createLiveLifecycle({ db, dbPath, pick, emojis, onError, isStable, chatbotResponder } = {}) {
  ...
  const modules = createAutomationModules({ config, pick, emojis, chatbotResponder });
  ...
  dispatcher.on('messages.upsert', async ({ sock, messages = [] }) => {
    for (const message of messages) {
      ...
      // Phase 4 owns all message-driven automations. This is the single
      // execution path; legacy index.js automation toggles are not authoritative.
      if (isStatus) {
