...
const { createPhase6Boundary, sessionLifecycleState, applySocketLifecycleState } = require('./phase6-lifecycle');
...
function createLiveLifecycle({ db, dbPath, pick, emojis, onError, isStable, chatbotResponder } = {}) {
  ...
  dispatcher.on('messages.update', async ({ sock, updates = [] } = {}) => {
    const settings = await config.get();
    if (!settings.global.antidelete) return { skipped: true, reason: 'disabled' };
    let recovered = 0;
    for (const update of updates) {
      ...
      if (!cached || cached.key?.fromMe) continue;
      const jid = cached.key?.remoteJid || cached.key?.remoteJidAlt || '';
      if (!jid || typeof sock?.sendMessage !== 'function') continue;
      const body = modules.extractText(cached);
      if (body) {
        await sock.sendMessage(jid, { text: '🗑️ *Deleted message recovered*\\n\\n' + body }, { quoted: cached });
        recovered += 1;
      }
    }
    return { recovered };
  ...
  dispatcher.on('connection.update', async ({ sock, connection, credsReady } = {}) => {
    ...
    if (sock) applySocketLifecycleState(sock, lifecycle);
  ...
  return {
    ...
    attach(sock) {
      ...
      sock.ev.on('connection.update', (payload) => dispatcher.emit('connection.update', { sock, ...payload }));
      return sock;
    },
  };
}
