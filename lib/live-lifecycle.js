 ...
 * never replaces the connection/auth/reconnect layer.
 *
 * `runAutomations` is opt-in because the current production index.js still
 * owns the legacy read/react/presence handlers. Keeping the modular lifecycle
 * passive by default prevents duplicate actions during the migration. Once
 * those legacy handlers are removed, the modular automation modules can be
 * enabled explicitly with runAutomations: true.
 */
function createLiveLifecycle({ db, dbPath, pick, emojis, onError, getSettings, isStable, runAutomations = false } = {}) {
  ...
  dispatcher.on('messages.upsert', async ({ sock, messages = [] }) => {
    ...
    for (const message of messages) {
      ...
      await moderation.processMessage(sock, message, settings);
