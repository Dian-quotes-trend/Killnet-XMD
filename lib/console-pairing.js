...
function formatOriginal(level, args) {
  ...
  // Keep unrelated application output visible, but remove the old box glyphs.
  ...
  baileys.useMultiFileAuthState = async function consoleAuthState(folder, ...args) {
    ...
    if (typeof originalSaveCreds === 'function') {
      result.saveCreds = async (...saveArgs) => {
        ...
        emit('AUTH', 'WhatsApp credentials updated and saved.');
      };
    }
    return result;
  };
  ...
  baileys.default = function consoleMakeWASocket(...args) {
    ...
    if (typeof originalRequestPairingCode === 'function') {
      socket.requestPairingCode = async (phoneNumber) => {
        ...
        try {
          // A short delay lets the initial WA handshake settle before the
          // pairing request. This is intentionally inside the wrapper so the
          // existing connection lifecycle remains untouched.
