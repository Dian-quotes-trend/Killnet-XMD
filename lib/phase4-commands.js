...
const ALIASES = Object.freeze({
  autostatusview: 'auto-status-view',
  autoviewstatus: 'auto-status-view',
  autolikestatus: 'auto-status-like',
  autoreactstatus: 'auto-status-like',
  autostatussave: 'auto-status-save',
...
function statusHandler(store, command) {
  return async (ctx) => {
    ...
    await store.update((value) => ({ ...value, status: { ...value.status, [command.key]: next } }));
...
function registerPhase4Commands(registry, { config } = {}) {
  ...
  for (const name of AUTOMATION_KEYS) {
    ...
    const handler = async (ctx) => {
      const arg = ctx.args?.[0];
      const current = await store.get();
      const enabled = name === 'chatbot' ? Boolean(current.chatbot?.global) : Boolean(current.global?.[name]);
      ...
      await store.update((value) => {
        if (name === 'chatbot') return { ...value, global: { ...value.global, chatbot: next }, chatbot: { ...value.chatbot, global: next } };
        return { ...value, global: { ...value.global, [name]: next } };
      });
