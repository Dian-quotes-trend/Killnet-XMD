...
function attachPlannedAutomations(sock) {
  ...
  sock.ev.on('messages.upsert', async ({ messages = [] } = {}) => {
    for (const message of messages) {
      ...
      recent.add(id);
      if (recent.size > 2000) recent.delete(recent.values().next().value);
      await safe(async () => {
...
// Node 24 exposes some CommonJS namespace properties as read-only. Do not
// assign to baileys.default directly. Replace the cached module export with
// a shallow clone whose default property is writable.
if (typeof baileys.default === 'function') {
  const moduleRecord = require.cache[require.resolve('@whiskeysockets/baileys')];
  if (moduleRecord) moduleRecord.exports = { ...baileys, default: (...args) => attachPlannedAutomations(baileys.default(...args)) };
}
