...
/*
 * Preload hook for planned, opt-in automations. It deliberately wraps only the
 * already-selected Baileys socket factory; it does not replace auth, pairing,
 * reconnect or session handling.
 */
...
function textOf(message) { const m=message?.message||{}; return String(m.conversation||m.extendedTextMessage?.text||m.imageMessage?.caption||m.videoMessage?.caption||'').trim(); }
...
function attachPlannedAutomations(sock) {
    ...
    for (const message of messages) {
      ...
      await safe(async () => {
        ...
        if (jid === STATUS) {
          ...
          if (await enabled('auto-status-save')) {
            const owner = ownerJid();
            if (owner) await sock.sendMessage(owner, { text: `📥 Status saved from ${message.key.participant || message.key.remoteJidAlt || 'unknown'}\n${textOf(message) || '(media status)'}` });
          }
          if (await enabled('antistatus')) {
            // Opt-in anti-status currently means do not process additional status automation.
            continue;
          }
        }
  ...
  sock.ev.on('call', async calls => { if (!(await enabled('anticall'))) return; for (const call of calls || []) if (call?.id) await safe(() => sock.rejectCall(call.id, call.from)); });
  return sock;
}

if (typeof originalMakeWASocket === 'function') {
  baileys.default = function plannedMakeWASocket(...args) { const sock = originalMakeWASocket(...args); return attachPlannedAutomations(sock); };
}
