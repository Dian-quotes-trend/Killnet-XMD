...
const live = createLegacyDispatchAdapter(commands, { getActor: (ctx) => ctx.actor });
const lifecycle = createLiveLifecycle({ dbPath: path.join(DATA_DIR, 'automation.json'), pick, emojis: settings.autoReactEmojis, isStable: () => runtimeState.lifecycleStable });
...
async function handleIncoming(sock, msg) {
  ...
  const parsed = text.startsWith(settings.prefix) ? { name: text.slice(settings.prefix.length).trim().split(/\s+/)[0]?.toLowerCase(), args: text.slice(settings.prefix.length).trim().split(/\s+/).slice(1) } : null;
  ...
  if (settings.autoReact && !msg.key.fromMe) await sock.sendMessage(jid, { react: { text: pick(settings.autoReactEmojis), key: msg.key } }).catch(() => {});
  if (settings.autoReply.enabled && text && !text.startsWith(settings.prefix) && !msg.key.fromMe) await send(sock, jid, settings.autoReply.message, msg).catch(() => {});
  if (!parsed) return;
